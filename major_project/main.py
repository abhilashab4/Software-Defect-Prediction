...
# 3. Training Loop
print(f"🚀 Training for {cfg.EPOCHS} Epochs (Seed: 42, Intensity: 1.5)...")
for epoch in range(cfg.EPOCHS):
    model.train()
    for m, s, y in train_loader:
        m, s, y = m.to(cfg.DEVICE), s.to(cfg.DEVICE), y.to(cfg.DEVICE)
...
# # ==========================================
# # 1. LOAD DATA (NO LEAKAGE)
# # ==========================================
# print(f"--- 🔄 Starting Experiment: {cfg.PROJECT_NAME.upper()} ---")

# train_paths = [
#     f"data/{cfg.PROJECT_NAME}/{cfg.PROJECT_NAME}_{v}_enriched.csv"
#     for v in cfg.TRAIN_VERSIONS
# ]
...
# # ==========================================
# # 2. 🔥 WEIGHTED SAMPLER (BALANCED BATCHES)
# # ==========================================
...
# # ❌ Removed pos_weight (sampler handles imbalance)
...
# # 4. TRAINING
# # ==========================================
# print(f"🚀 Training for {cfg.EPOCHS} Epochs (Balanced + No Leakage)...")
...
#                 y,
#                 torch.ones(len(bug_idx)).to(cfg.DEVICE)
#             ])
#         else:
#             all_logits, all_targets = logits.view(-1), y
...
#         loss = criterion(all_logits, all_targets) + cfg.KL_COEFF * kl_loss
...
#             y_pred_epoch.extend(
#                 (torch.sigmoid(l_t) > 0.5).cpu().numpy()
#             )
#             y_true_epoch.extend(y_t.numpy())
...
#     # t-SNE
#     mu, _, _ = model(
#         test_ds.X_metrics.to(cfg.DEVICE),
#         test_ds.X_seq.to(cfg.DEVICE)
#     )

#     z_2d = TSNE(n_components=2, random_state=42).fit_transform(
#         mu.cpu().numpy()
#     )
...
#     alpha=0.7
# )
# plt.title(f"{cfg.PROJECT_NAME.upper()} Latent Space (F1: {best_f1:.4f})")
# plt.savefig(f"results/{cfg.PROJECT_NAME}_tsne.png")

# print(f"✅ Finished. Best F1: {best_f1:.4f}")